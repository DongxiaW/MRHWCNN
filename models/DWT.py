...
def dwt(x):
    ...
    #sampling rotation angle theta
    theta = K.random_normal(shape=(1,), mean = 0.0, stddev = 1.) #0.5

    x_LL = K.square(K.cos(theta)-K.sin(theta))*x1 + \
    (K.square(K.cos(theta))-K.square(K.sin(theta)))*x2 + \
    (K.square(K.cos(theta))-K.square(K.sin(theta)))*x3 + \
    K.square(K.cos(theta)+K.sin(theta))*x4

    x_LH = -(K.square(K.cos(theta))-K.square(K.sin(theta)))*x1 - \
    K.square(K.cos(theta)+K.sin(theta))*x3 + \
    K.square(K.cos(theta)-K.sin(theta))*x2 + \
    (K.square(K.cos(theta))-K.square(K.sin(theta)))*x4

    x_HL = -(K.square(K.cos(theta))-K.square(K.sin(theta)))*x1 + \
    K.square(K.cos(theta)-K.sin(theta))*x3 - \
    K.square(K.cos(theta)+K.sin(theta))*x2 + \
    (K.square(K.cos(theta))-K.square(K.sin(theta)))*x4

    x_HH = K.square(K.cos(theta)+K.sin(theta))*x1 - \
    (K.square(K.cos(theta))-K.square(K.sin(theta)))*x3 - \
    (K.square(K.cos(theta))-K.square(K.sin(theta)))*x2 + \
    K.square(K.cos(theta)-K.sin(theta))*x4
    ...
    #elif data_format == 'channels_first':
    #    return K.concatenate([x_LL,x_LH,x_HL,x_HH],axis=1)
    return K.concatenate([x_LL,x_LH,x_HL,x_HH],axis=-1), theta
...
def iwt(x, theta):
    ...
    # x3 = (x_LL + x_LH - x_HL - x_HH)/4
    # x4 = (x_LL + x_LH + x_HL + x_HH)/4
    x1 = (x_LL*K.square(K.cos(theta)-K.sin(theta)) - x_LH*(K.square(K.cos(theta))-K.square(K.sin(theta))) - \
        x_HL*(K.square(K.cos(theta))-K.square(K.sin(theta))) + x_HH*K.square(K.cos(theta)+K.sin(theta)))/4
    x2 = (x_LL*(K.square(K.cos(theta))-K.square(K.sin(theta))) - x_LH*K.square(K.cos(theta)+K.sin(theta)) + \
        x_HL*K.square(K.cos(theta)-K.sin(theta)) - x_HH*(K.square(K.cos(theta))-K.square(K.sin(theta))))/4
    x3 = (x_LL*(K.square(K.cos(theta))-K.square(K.sin(theta))) + x_LH*K.square(K.cos(theta)-K.sin(theta)) - \
        x_HL*K.square(K.cos(theta)+K.sin(theta)) - x_HH*(K.square(K.cos(theta))-K.square(K.sin(theta))))/4
    x4 = (x_LL*K.square(K.cos(theta)+K.sin(theta)) + x_LH*(K.square(K.cos(theta))-K.square(K.sin(theta))) + \
        x_HL*(K.square(K.cos(theta))-K.square(K.sin(theta))) + x_HH*K.square(K.cos(theta)-K.sin(theta)))/4
